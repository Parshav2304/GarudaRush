...
@alerts_bp.route('/info', methods=['GET'])
def alerts_info():
    """Return available alerts endpoints (public info)"""
    return jsonify({
        'message': 'GarudaRush Alerts API',
        'endpoints': {
            'list': '/api/alerts/',
            'create': '/api/alerts/create',
            'summary': '/api/alerts/summary',
            'trends': '/api/alerts/trends',
    ...
    try:
        ...
        for alert in alerts:
            alert_list.append({
                ...
                'acknowledged_at': alert.get('acknowledged_at').isoformat() + 'Z' if alert.get('acknowledged_at') else None,
