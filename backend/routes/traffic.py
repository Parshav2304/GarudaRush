...
def traffic_info():
    """Return available traffic endpoints"""
    return jsonify({
        'message': 'GarudaRush Traffic Monitoring API',
        'endpoints': {
            'stats': '/api/traffic/stats',
            'live': '/api/traffic/live',
            'history': '/api/traffic/history',
            'submit': '/api/traffic/submit',
            'export': '/api/traffic/export'
        }
    }), 200
...
def get_live_traffic():
    ...
    try:
        ...
        for data in traffic_data:
            point = {
                'time': data['timestamp'].isoformat() + 'Z',
